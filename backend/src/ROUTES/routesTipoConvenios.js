...
router.post('/validartipoconvenio', bodyParser.json() , (req , res)=>{
    const { nombre } = req.body;
    console.log(nombre)
            mysqlConect.query('SELECT * FROM tipo_convenios WHERE nombre=?', [nombre], (error, registros)=>{
                if(error){
                    console.log('Error en la base de datos', error)
                }else{
                    ...
                    if(registros.length>0){
                        res.json({
                            status:true,
                            mensaje:"El nombre de tipo de convenio ya existe" 
                        })
                    }else{
                        res.json({
                            status:false,
